...
<div class="modal-body">
    <div id="amountInputs" class="mb-4"></div> <!-- Moved up for better organization -->
        ...
        <div class="row my-2">
            @foreach($chunk as $digit)
                <div class="col-2 text-center digit" onclick="selectDigit('{{ $digit->two_digit }}', this)">
                    {{ $digit->two_digit }}
                    <input type="number" id="amount_{{ $digit->two_digit }}" name="amounts[{{ $digit->two_digit }}]" class="form-control mt-2 d-none" placeholder="Amount for {{ $digit->two_digit }}" min="100" max="5000" onchange="updateTotalAmount();"> <!-- Added d-none class to initially hide it -->
                </div>
            @endforeach
        </div>
    @endforeach
    ...
    <form action="" method="post">
        ...
        <input type="hidden" name="selected_digits" id="selected_digits"> <!-- Changed to type="hidden" -->
